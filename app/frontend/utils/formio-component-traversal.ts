...
import { escapeForSingleQuotedJsString, urlForPath } from "./utility-functions"
...
export const readValue = (submissionData: any, path: (string | number)[]) => R.path(path, submissionData)
...
const resolveComponentDisplay = (component: any, value: any): string => {
  const type = component?.requirementInputType || component?.type
  switch (type) {
