...
export const StepCodeOccupancies = ({ checklist }: IProps) => {
  const i18nPrefixDetails = `${i18nPrefix}.stepCodeOccupancies`
  return (
    <Panel heading={t(`${i18nPrefixDetails}.heading`)}>
      <Field
        label={t(`${i18nPrefixDetails}.isRelevant`)}
        value={!R.isEmpty(checklist.stepCodeOccupancies) ? "Yes" : "No"}
      />
    </Panel>
  )
}
