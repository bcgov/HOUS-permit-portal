...
export const JurisdictionSubmissionInboxScreen = observer(function JurisdictionSubmissionInbox() {
  ...
  useSearch(permitApplicationStore, [currentJurisdiction?.id, showArchived])
  ...
  return (
    <Container maxW="container.lg" p={8} as={"main"}>
      <VStack alignItems={"flex-start"} spacing={5} w={"full"} h={"full"}>
                ...
                <Box key={u.id} className={"jurisdiction-user-index-grid-row"} role={"row"} display={"contents"}>
                  ...
                  <SearchGridItem>
                    <Flex justify="space-between" w="full">
                      <ManageUserMenu user={u} searchModel={permitApplicationStore} />
                    </Flex>
                  </SearchGridItem>
                </Box>
              )
        ...
        <ToggleArchivedButton searchModel={permitApplicationStore} mt={3} />
      </VStack>
    </Container>
  )
})
