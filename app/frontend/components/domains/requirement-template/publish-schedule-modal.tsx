import { Alert, Box, Button, ButtonProps, Dialog, Portal, Stack, Text, useDisclosure } from "@chakra-ui/react"
    ...
    <>
      ...
      >
        <Portal>
          <Dialog.Backdrop />
          <Dialog.Positioner>
            <Dialog.Content w="full" maxW={onForcePublishNow || onCreateDraft ? "680px" : "436px"} mx={4}>
              ...
              <Dialog.Body>
                ...
                {conflictsOnOrBeforeSelected.length > 0 && (
                  <Alert.Root status="warning" mt={4} borderRadius="md" alignItems="flex-start">
                ...
                {laterScheduled.length > 0 && (
                  <Alert.Root status="info" mt={4} borderRadius="md" alignItems="flex-start">
                    <Alert.Indicator mt={1} />
                    <Text>
                      <Trans
                        // @ts-ignore
              ...
              <Dialog.Footer justifyContent={"flex-start"} mt={4} gap={3} flexWrap={"wrap"}>
                <Button variant={"primary"} disabled={!scheduleDate} onClick={onSchedule}>
                  {t("ui.confirm")}
                </Button>
      ...
      <Dialog.Root
        ...
        initialFocusEl={() => overrideCancelRef.current}
        role="alertdialog"
        onOpenChange={(e) => {
          if (!e.open) {
            overrideDialog.onClose()
