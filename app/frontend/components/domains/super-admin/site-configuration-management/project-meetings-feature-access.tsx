...
export const ProjectMeetingsFeatureAccessScreen = observer(() => {
  ...
  const updateProjectMeetingsEnabled = async (checked: boolean) => {
    await updateSiteConfiguration({ projectMeetingsEnabled: checked })
  }
      ...
      <Flex justifyContent={"space-between"} w={"full"} alignItems={"flex-end"}>
        <VStack justifyContent={"space-between"} w={"full"} alignItems={"flex-end"}>
          <Flex direction="row" justify="space-between" w="100%">
            <Text style={{ fontWeight: "bold" }}>{t(`${i18nPrefix}.projectMeetings`)}</Text>
            <HStack spacing={5} align="center">
              <SwitchButton
                isChecked={siteConfigurationStore.projectMeetingsEnabled || false}
                onChange={(e) => updateProjectMeetingsEnabled(e.target.checked)}
                size={"lg"}
              />
            </HStack>
          </Flex>
