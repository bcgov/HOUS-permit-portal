class SubmissionVersion < ApplicationRecord
  ...
  # Setting default_scope to filter non-sandboxed SubmissionVersions by default
  default_scope { joins(:permit_application).where(permit_applications: { sandboxed: false }) }
