  ...
  describe "#create_integration_mappings callback" do
    context "when the template version is published and status has changed" do
      ...
      it "creates integration requirements mappings for jurisdictions without mapping and whose api keys are enabled" do
        expect { template_version.update(status: "published") }.to change { IntegrationMapping.count }.by(1)

        expect(IntegrationMapping.find_by(template_version: template_version, jurisdiction: jurisdiction)).to be_present

        expect(IntegrationMapping.find_by_jurisdiction_id(disabled_api_jurisdiction.id)).not_to be_present
      end
    end
    ...
    context "when the template version is not published" do
      ...
      it "does not create integration requirements mappings" do
        expect { template_version.save }.not_to change { IntegrationMapping.count }
      end
    end
    ...
    context "when the template version status has not changed" do
      ...
      it "does not create integration requirements mappings" do
        expect { template_version.touch }.not_to change { IntegrationMapping.count }
      end
    end
  end
end
