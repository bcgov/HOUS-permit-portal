...
RSpec.describe Api::ContactsController, type: :controller do
  ...
  let!(:contacts) { create_list(:contact, 5, contactable: submitter, first_name: "searchme") }
  let!(:other_contacts) { create_list(:contact, 5, contactable: other_submitter, first_name: "searchme") }
  let!(:jurisdiction_contacts) { create_list(:contact, 5, contactable: jurisdiction, first_name: "searchme") }
  ...
  describe "GET #contact_options" do
    context "It only returns the current users contacts as options" do
      it "returns a successful response with the correct data structure" do
        ...
        expect(json_response["data"].map { |opt| opt["value"] }.pluck("id")).to match_array(contacts.pluck(:id))
      end
    end
  end
  ...
  describe "POST #create" do
    context "It only returns the created contact only to the correct user" do
      it "returns a successful response with the correct data structure" do
        ...
        get :contact_options, params: { query: "searchme" }
        expect(json_response["data"].map { |opt| opt["value"] }.pluck("id")).to match_array(other_contacts.pluck(:id))
      end
    end
  end
end
