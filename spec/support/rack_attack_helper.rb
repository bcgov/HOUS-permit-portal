...
module RackAttackHelper
  def with_temporary_rate_limit(throttle_key, limit:, period:, &block)
    ...
    Rack::Attack.throttle(throttle_key, limit: limit, period: period) { |req| req.ip }

    block.call

    # Revert to the original configuration
    Rack::Attack.throttle(throttle_key, limit: original_limit, period: original_period) { |req| req.ip }
    Rack::Attack.reset!
  end
end
