...
help_video_file_data =
  lambda do |filename:, mime_type:, size: 1.kilobyte|
    {
...
seed_help_video_document =
  lambda do |video:, klass:, filename:, mime_type:, size: 1.kilobyte|
    document = klass.find_or_initialize_by(help_video: video)
    document.assign_attributes(
      file_data:
        help_video_file_data.call(
          filename: filename,
          mime_type: mime_type,
          size: size
        ),
...
help_video_sections = [
  ...
  },
  {
    ...
    description: "Track application progress and respond to reviewer feedback.",
    videos: [
      {
...
help_video_sections.each_with_index do |section_attributes, section_index|
  section =
    HelpVideoSection.find_or_initialize_by(title: section_attributes[:title])
  section.description = section_attributes[:description]
  section.save!
  section.insert_at(section_index)

  section_attributes[:videos].each_with_index do |video_attributes, video_index|
    video = HelpVideo.find_or_initialize_by(title: video_attributes[:title])
    video.assign_attributes(
      help_video_section: section,
      description: video_attributes[:description],
      published_at: nil
    )
    video.save!
...
puts "  ✓ Seeded #{help_video_sections.size} help video sections"
