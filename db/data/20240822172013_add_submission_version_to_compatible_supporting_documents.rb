...
class AddSubmissionVersionToCompatibleSupportingDocuments < ActiveRecord::Migration[7.1]
  def up
    pdf_documents =
      SupportingDocument
        .where(data_key: SupportingDocument::APPLICATION_PDF_DATA_KEY, submission_version_id: nil)
        .or(SupportingDocument.where(data_key: SupportingDocument::CHECKLIST_PDF_DATA_KEY, submission_version_id: nil))
  ...
  def down
    pdf_documents =
      SupportingDocument
        ...
        .or(SupportingDocument.where(data_key: SupportingDocument::CHECKLIST_PDF_DATA_KEY))
